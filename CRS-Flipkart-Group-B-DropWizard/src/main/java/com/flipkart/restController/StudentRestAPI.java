...
public class StudentRestAPI {
	...
	@Produces(MediaType.APPLICATION_JSON)
	public Response payFee(@PathParam("studentID") String studentID, @QueryParam("mode") String mode, @QueryParam("credentials") String credentials) {
		try{
			if(!registrationInterface.isRegistrationDone(studentID)) {
				...
				int amount = registrationInterface.calculateFee(studentID);
				registrationInterface.payFee(studentID, ModeOfPaymentConstant.valueOf(mode), amount, credentials);
